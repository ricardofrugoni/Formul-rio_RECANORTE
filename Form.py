...
st.image("pucminas_logo.png", width=120)
...
if tipo_pessoa == "Pessoa Física":
    ...
    respostas["NPS"] = st.radio("De 0 a 10, quanto você indicaria a RECANORTE para um amigo?", list(range(11)), horizontal=True)
...
# === PESSOA JURÍDICA ===
    ...
    respostas["NPS"] = st.radio("De 0 a 10, quanto você indicaria a RECANORTE a outro empreendedor?", list(range(11)), horizontal=True)
    ...
    for item in ["Qualidade", "Preço", "Condições de pagamento", "Prazo de entrega", "Variedade", "Atendimento vendas", "Pós-venda"]:
        respostas[f"Avaliação: {item}"] = st.radio(item, list(range(1, 6)), horizontal=True)
